...
### Added
- **Trends browse: filter by source, sort, pagination** — the trends page gains source radio
  buttons (only connectors with data), a sort selector (Newest / Highest opportunity), and pagination.
  New `listTrendsPage()` in `@aioi/database`; a client `TrendControls` updates the URL; responsive.
