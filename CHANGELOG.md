...
### Added
- **Build kit (scaffold export)** — a trend's action plan is assembled into a ready-to-paste scaffold
  prompt for an AI coding agent (Claude Code / Cursor / v0), with copy + download, on the detail page.
  Deterministic `buildScaffoldPrompt` (@aioi/shared) — the last mile of "signal → shipped".
