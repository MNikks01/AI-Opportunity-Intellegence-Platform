...
| `SEC_USER_AGENT`                            | Enables the **SEC EDGAR Form D** funding source (M15-B). No API key — SEC's fair-access policy just requires a contact User-Agent, e.g. `AIOI research you@example.com`. **Wired**: ingests AI private-funding filings (leading demand signal); no-ops without it. **US-only** (Form D is a US filing).                         |
| `CRUNCHBASE_API_KEY`                        | Enables the **Crunchbase** global funding source (LICENSED — [ADR-0008](../adr/ADR-0008-global-funding-source.md)). **Requires a paid Crunchbase Data API license.** **Wired + inert**: no calls/cost without the key; set it and global AI funding rounds flow through the pipeline onto `/funding` + `/market` automatically. |
