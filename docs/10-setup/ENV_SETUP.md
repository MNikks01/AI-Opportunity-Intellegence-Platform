...
Only needed to ingest from that source. **HackerNews needs no key.** See the `data-source-integration`
skill — official/licensed sources only.

| Variable                                    | Where to get it                                                                                                         |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `GITHUB_TOKEN`                              | github.com → Settings → **Developer settings → Personal access tokens** → _fine-grained token_ (public read is enough). |
| `REDDIT_CLIENT_ID` / `REDDIT_CLIENT_SECRET` | reddit.com/prefs/apps → _create app_ (type **script**) → id under the name, secret shown once. Official API only.       |
